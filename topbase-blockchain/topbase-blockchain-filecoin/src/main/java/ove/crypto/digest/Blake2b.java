...
public interface Blake2b {
	...
	public static class Tree {
		 ...
		 * @param fanout
		 * @param depth
		 * @param leafLength size of data input for leaf nodes.
		 * @param innerLength note this is used also as digest-length for non-root nodes.
		 * @param digestLength final hash out digest-length for the tree
		 */
		public Tree  (
				final int     depth,
				final int     fanout,
				final int     leafLength,
				final int     innerLength,
				final int     digestLength
		) {
			this.fanout = fanout;
			this.depth = depth;
			this.leaf_length = leafLength;
			this.inner_length = innerLength;
			this.digest_length = digestLength;
		}
	...
	static class Engine implements Blake2b {
		...
		private void initialize () {
			...
			System.arraycopy( param.initializedH(), 0, this.state.h, 0, Spec.STATE_SPACE_LEN);

			// if we have a key update initial block
			// Note param has zero padded key_bytes to Spec.max_key_bytes
			if(param.hasKey){
				this.update (param.keyBytes, 0, Spec.BLOCK_BYTES);
			}
		}
		...
		@Override final public void digest(byte[] output, int off, int len) {
			...
			this.state.f[ Flag.last_node ] = this.state.lastNode ? 0xFFFFFFFFFFFFFFFFL : 0x0L;
		...
		private void compress (final byte[] b, final int offset) {
			...
			round8(v, m);
			round9(v, m);
			round0(v, m); // round 10 is identical to round 0
			round1(v, m); // round 11 is identical to round 1
		...
		private void round0(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[0];
			v[12] ^= v[ 0];
		...
		private void round1(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[14];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round2(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[11];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round3(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[7];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round4(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[9];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round5(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[2];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round6(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[12];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round7(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[13];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round8(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[6];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		private void round9(final long[] v, final long[] m) {
			v[ 0] = v[ 0] + v[ 4] + m[10];
			v[12] ^= v[ 0];
			v[12] = ( v[12] << 32 ) | ( v[12] >>> 32 );
			v[ 8] = v[ 8] + v[12];
		...
		public static class LittleEndian {
			...
			static public String toHexStr (final byte[] b, boolean upperCase) {
				...
				final byte[] hexRep = upperCase ? HEX_digits : hex_digits ;
				for (int i = 0; i < len; i++) {
					digits [ i*2   ] = hexRep [ (byte) (b[i] >> 4 & 0x0F)  ];
					digits [ i*2+1 ] = hexRep [ (byte) (b[i]      & 0x0F) ];
				}
	...
	public static class Param implements AlgorithmParameterSpec {
		...
		final byte getByteParam (final int xoffset) {
			...
			if(bytes == null){
				bytes = Param.default_bytes;
			}
			return bytes[ xoffset];
		}
		...
		@Override public Param clone() {
			...
			if(this.hasKey){
				...
				clone.keyBytes = new byte [Spec.MAX_KEY_BYTES * 2];
				System.arraycopy(this.keyBytes, 0, clone.keyBytes, 0, this.keyBytes.length);
			}
			return clone;
		}
		////////////////////////////////////////////////////////////////////////
		...
		public final Param setKey (final byte[] key) {
			...
			// zeropad keybytes
			this.keyBytes = new byte [Spec.MAX_KEY_BYTES * 2];
		...
		public final Param setLeafLength(int leafLength) {
			assert leafLength >= 0 : assertFail("leaf_length", leafLength, inclusiveLowerBound, 0);

			lazyInitBytes();
			writeInt (leafLength, bytes, Xoff.leaf_length);
			h[ 0 ] = readLong( bytes, 0  );
			h[ 0 ] ^= Spec.IV [ 0 ];
			return this;
		}
		...
		/* 8-15 inclusive */
		public final Param setNodeOffset(long nodeOffset) {
			assert nodeOffset >= 0 : assertFail("node_offset", nodeOffset, inclusiveLowerBound, 0);

			lazyInitBytes();
			writeLong(nodeOffset, bytes, Xoff.node_offset);
		...
		/* 16-23 inclusive */
		public final Param setNodeDepth(int nodeDepth) {
			assert nodeDepth >= 0 : assertFail("node_depth", nodeDepth, inclusiveLowerBound, 0);

			lazyInitBytes();
			bytes[ Xoff.node_depth ] = (byte) nodeDepth;
		...
		public final Param setInnerLength(int innerLength) {
			assert innerLength >= 0 : assertFail("inner_length", innerLength, inclusiveLowerBound, 0);

			lazyInitBytes();
			bytes[ Xoff.inner_length] = (byte) innerLength;
			h[ 2 ] = readLong( bytes, Xoff.node_depth );
			h[ 2 ] ^= Spec.IV [ 2 ];
			h[ 3 ] = readLong( bytes, Xoff.node_depth + 8);
			h[ 3 ] ^= Spec.IV [ 3 ];
